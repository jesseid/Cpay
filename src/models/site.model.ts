...
export async function getCommentsBySite(input: {
  ...
  notes.list = notes.list.filter((item) =>
    item.toNote?.metadata?.content?.sources?.includes("chainmeta"),
  )

  return notes
}

const chainmetaOperatorPermissions: CharacterOperatorPermission[] = [
...
export async function addOperator(
  ...
  if (input.operator && input.characterId) {
    return contract?.operator.grantForCharacter({
      ...
      permissions: chainmetaOperatorPermissions,
    })
  }
}
...
export async function getOperators(input: { characterId?: number }) {
  if (input.characterId) {
    ...
    result.list = result.list
      ...
      .filter((o) => {
        for (const permission of chainmetaOperatorPermissions) {
...
export async function getAchievements(characterId: number) {
  ...
  const chainmetaAchievements: AchievementSection[] = [
    {
      info: {
        name: "chainmeta-journey",
        title: "chainmeta Journey",
      },
      groups: [
        {
          info: {
          ...
          items: [
            {
              info: {
                ...
                description: "I am a superstar on chainmeta!",
                media:
          ...
          items: [
            {
              info: {
                ...
                description: "I migrated from Mirror.xyz to chainmeta!",
                media:
  ...
  return {
    list: [...chainmetaAchievements, ...(crossbellAchievements || [])],
  }
}
...
export async function fetchTenant(
  ...
  const res = await fetch(
    `https://cloudflare-dns.com/dns-query?name=_chainmeta-challenge.${host}&type=TXT`,
    {
      headers: {
