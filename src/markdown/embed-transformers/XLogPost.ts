...
function getParamsFromShortsURL(url: URL) {
  ...
  // https://caspian-3030.chainmeta.app/AtOAglnMV_N6xslDbfhz4?ct=shorts
  ...
  if (matched) {
    ...
    // https://chainmeta.app/site/caspian-3030/AtOAglnMV_N6xslDbfhz4?ct=shorts
...
export const chainmetaPostTransformer: Transformer = {
  name: "chainmetaShorts",
  shouldTransform(url) {
    const { host } = url
    return (
  ...
  getHTML(url) {
    ...
    return `<chainmeta-post slug="${slug}" handle="${handle}" url="${url.toString()}"/>`
  },
}
