    ...
    <PostModal handle={site?.handle}>
      <div className="pb-16 overflow-x-hidden overflow-y-scroll">
        ...
        <main
          ...
          className={cn(
            `chainmeta-post-id-${page?.characterId}-${page?.noteId}`,
            "chainmeta-deprecated-class chainmeta-post-area max-w-screen-md mx-auto px-5 pt-12 relative",
          )}
        >
          <article>
            {type === "short" ? (
              <>
                ...
                {page?.metadata?.content?.title && (
                  <h2 className="chainmeta-short-title font-bold mb-2 text-lg">
                    {page?.metadata?.content?.title}
                  </h2>
                )}
                <div className="chainmeta-short-content prose">
